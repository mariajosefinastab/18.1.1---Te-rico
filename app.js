...
const mariadb = require("mariadb"); //Require para importar librería

const pool = mariadb.createPool({ //Crea conexiónes a base de datos 
  host: "localhost",
  user: "root",
  password: "1234",
  database: "pruebadb",
...
app.get("/people", async (req, res) => { //declaro async para poder usar await
  let conn;
  try {
    conn = await pool.getConnection(); //await de response
